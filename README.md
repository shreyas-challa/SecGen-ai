...
- **Shell command execution**: Run arbitrary commands, manage background processes (reverse shell listeners, long-running scans)
...
| `LHOST` | (empty) | Your attacker IP (tun0 on HTB VPN) for reverse shells |
...
| `ALLOWED_SCOPE` | (empty) | Comma-separated IPs, CIDRs, or domains |
...
| `shell_command` | Arbitrary shell execution with background process management |
| `generate_report` | Final markdown report generation |

### Shell Command Tool

The `shell_command` tool supports four actions:
...
agent.py             Claude tool-use loop + shell tracking
...
tools/
  ...
  shell_tool.py      Shell command execution + background process mgmt
...
5. **Report is generated** — structured markdown with all findings, attack chain, and proof
6. **Shell handoff** — if background processes (listeners, shells) are still active, connection info is printed
